...
def plot_time_series(df, title):
    ...
    for i, date in enumerate(unique_dates):
        ...
        axs[i].plot(daily_df.index, daily_df['GroundTruthFloor'], '-', linewidth=2.5, label='Floor: Ground Truth')
        axs[i].plot(daily_df.index, daily_df['Floor'], '--', linewidth=2.5, label='Floor: Wi-Fi Raw')
        axs[i].plot(daily_df.index, daily_df['Corrected_Floor'], ':', linewidth=3, label='Floor: Wi-Fi Corrected')

        # Sub-plot title labels
        title_font = {"family": "Times New Roman", "size":22, "weight": "bold"}
        axs[i].set_title(f'{date.strftime("%Y-%m-%d")} (RMSE: {rmse:.2f})', fontdict=title_font)
        ...
        # set labels for first figure only.v
        if i == 0:
            legend_font = {"family": "Times New Roman", "size": 17}
            axs[i].legend(loc='best', prop=legend_font)

            # X Y labels
            xy_font = {"family": "Times New Roman", "size": 20}
            axs[i].set_xlabel('Datetime', fontdict=xy_font)
            axs[i].set_ylabel('Floor', fontdict=xy_font)
