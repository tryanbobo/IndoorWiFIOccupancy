...
filtered_data = pd.read_csv(os.path.join(path, 'Mobility(Dynamic)_Non-humanFilter.csv'))

# Convert Datetime column to datetime type and round to 30-minute intervals
filtered_data['Datetime'] = pd.to_datetime(filtered_data['Datetime']).dt.round('30min')
...
# Merge all_combinations with pivoted_data to ensure every floor is represented at each 30-minute interval
...
building_totals = merged_data.groupby('Datetime').sum().reset_index().drop(columns=["Corrected_Floor"])

# Save the DataFrame to a CSV file
building_totals.to_csv(os.path.join(path, 'agg/BuildingCounts_30min_dynamic.csv'), index=False)
...
ax.legend()
ax.set_title('Counts of Building Total at Every 30-minute Interval: Dynamic Filter')
