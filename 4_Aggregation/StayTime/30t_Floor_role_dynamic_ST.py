...
data = pd.read_csv(os.path.join(path, 'Mobility(Dynamic)_Non-humanFilter.csv'))
...
results_df.to_csv(path + r"\agg\aggDuration\FloorCounts_30min_dynamic.csv", index=False)
...
plt.title('Occupancy count for each floor at 30-minute intervals: Dynamic Filter')
plt.show()